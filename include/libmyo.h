...
/// Initiate pairing with a device that's very close (likely touching) the host device.
/// @returns libmyo_success if pairing initiated successfully, otherwise
///  - libmyo_error if pairing has already been initiated
///  - libmyo_error_invalid_argument if \a hub is NULL
LIBMYO_EXPORT
libmyo_result_t libmyo_pair_adjacent(libmyo_hub_t hub, libmyo_error_details_t* out_error);
